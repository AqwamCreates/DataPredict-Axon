...
From the previous [tutorial](GeneralTensorConventions.md), we can see that the dimension size array holds a number of important information. However, these are not enough to fully understand on how to use them when spatial layers are involved. 

Here are the list of spatial function layers that are available in this library:

* Convolution layers

* Pooling layers

In this tutorial, I will further explain what does the "spatial" mean and how these spatial function layers affects the dimension size array of any input tensors and output tensors.
...
You may already have seen that the convolutional layers and pooling layers contains the 1D, 2D and 3D. Those N-D dimensions refers to the spatial dimensions. Hence, the input tensor's dimension size array must contain those spatial dimensions. For example:
...
Now, you understand why the convolution layers and pooling layers generates an error when you supply them an input tensor that has incorrect number of dimensions.

### The Number Of Kernels

This determines the number of channels that will be produced for the output tensor, regardless of the number of channels from the input tensor. So, if we have 3 kernels, then it will produce an output tensor that has 3 channels. Pretty simple, right?
...
That is all for this tutorial. I do hope you understand what the spatial dimensions are and why the spatial layers requires specific number of dimensions for our input tensor. 

Now, go play around with the spatial layers since you now have this knowledge.
